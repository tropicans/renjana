    ...
    return (
        <div className="min-h-screen bg-background-light text-[#111418] transition-colors duration-300 dark:bg-background-dark dark:text-white">
            ...
            <main className="mx-auto max-w-[1200px] px-6 pb-20 pt-24">
                <section className="mb-14 grid gap-8 rounded-[32px] border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-cyan-50 p-8 shadow-sm dark:border-sky-900/40 dark:from-slate-950 dark:via-slate-900 dark:to-sky-950 lg:grid-cols-[1.3fr_0.7fr]">
                    <div className="space-y-6">
                        <span className="inline-flex items-center gap-2 rounded-full bg-sky-100 px-4 py-1 text-xs font-bold uppercase tracking-[0.2em] text-sky-700 dark:bg-sky-500/10 dark:text-sky-300">
                            <Sparkles className="h-3.5 w-3.5" /> {t.eventsPage.featuredEvent}
                        </span>
                        <div className="space-y-4">
                            <span className="inline-flex items-center gap-2 rounded-full bg-white/70 px-4 py-1 text-xs font-bold uppercase tracking-[0.2em] text-slate-500 dark:bg-slate-900/60 dark:text-slate-300">
                                {t.eventsPage.batchAndEvent}
                            </span>
                            <h1 className="max-w-3xl text-4xl font-extrabold tracking-tight md:text-6xl">
                                {t.eventsPage.discoverBatches}
                            </h1>
                            <p className="max-w-2xl text-lg text-slate-600 dark:text-slate-300">
                                {t.eventsPage.discoverSubtitle}
                            </p>
                        </div>
                        <div className="flex flex-wrap gap-3 text-sm text-slate-500 dark:text-slate-400">
                            <span className="rounded-full border border-slate-200 px-4 py-2 dark:border-slate-800">{t.eventsPage.activeBatches}</span>
                            <span className="rounded-full border border-slate-200 px-4 py-2 dark:border-slate-800">{t.eventsPage.onlineReg}</span>
                            <span className="rounded-full border border-slate-200 px-4 py-2 dark:border-slate-800">{t.eventsPage.hybridLearn}</span>
                            <span className="rounded-full border border-slate-200 px-4 py-2 dark:border-slate-800">{t.eventsPage.evalCert}</span>
                        </div>
                    </div>

                    {featured && (
                        <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950/70">
                            <p className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">{t.eventsPage.currentlyOpen}</p>
                            ...
                            <div className="mt-6 grid gap-3 text-sm text-slate-600 dark:text-slate-300">
                                <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-sky-500" /> {featured.eventStart ? new Date(featured.eventStart).toLocaleDateString(dateLocale) : t.eventsPage.tba}</div>
                                ...
                                <div className="flex items-center gap-2"><Users className="h-4 w-4 text-sky-500" /> {featured._count.registrations} {t.eventsPage.registrantsCount}</div>
                            </div>
                            <div className="mt-6 rounded-2xl bg-slate-50 p-4 text-sm dark:bg-slate-900">
                                <p className="font-semibold">{t.eventsPage.feeStarting}</p>
                ...
                {error && <div className="rounded-2xl border border-red-200 bg-red-50 px-6 py-8 text-center text-red-600 dark:border-red-900/40 dark:bg-red-950/20">{t.eventsPage.failedToLoad}</div>}

                {!isLoading && !error && (
                    <div className="space-y-12">
                        <div className="flex items-end justify-between gap-4">
                            <div>
                                <h2 className="text-3xl font-extrabold tracking-tight">{t.eventsPage.listTitle}</h2>
                                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">{t.eventsPage.listSubtitle}</p>
                            </div>
                            <div className="text-sm text-slate-400">{events.length} {t.eventsPage.eventsCount}</div>
                        </div>
                        ...
                        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                            {(featured ? [featured, ...restEvents] : restEvents).map((event) => (
                                <article key={event.id} className="flex h-full flex-col rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-lg dark:border-slate-800 dark:bg-slate-950/70">
                                    ...
                                    <p className="flex-1 text-sm leading-6 text-slate-500 dark:text-slate-400">{event.summary || event.description || t.eventsPage.detailsAnnouncedSoon}</p>
                                    <div className="mt-6 space-y-2 text-sm text-slate-600 dark:text-slate-300">
                                        <div className="flex items-center gap-2"><CalendarDays className="h-4 w-4 text-primary" /> {t.eventsPage.regUntil} {event.registrationEnd ? new Date(event.registrationEnd).toLocaleDateString(dateLocale) : t.eventsPage.tba}</div>
                                        <div className="flex items-center gap-2"><Clock3 className="h-4 w-4 text-primary" /> {event.scheduleSummary || t.eventsPage.scheduleTba}</div>
                                        <div className="flex items-center gap-2"><Users className="h-4 w-4 text-primary" /> {event._count.registrations} {t.eventsPage.registrantsCount}</div>
                                    </div>
                                    <div className="mt-6 flex items-center justify-between gap-4">
                                        <div>
                                            <p className="text-xs text-slate-400">{t.eventsPage.onlineFeeStarting}</p>
